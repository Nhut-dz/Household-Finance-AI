"""Chạy train Bagging cho ML01 (F03 task 8).

    python scripts/train_bagging.py                  # tham số mặc định
    python scripts/train_bagging.py --rows 2000      # chạy nhanh khi thử
    python scripts/train_bagging.py --no-save        # không ghi ra runs/

Script này CHỈ là entry-point: mọi logic nằm ở
`hfml.ml.ml01_recommendation.train`. Giữ nó mỏng để phần chạy được và phần
được kiểm thử là cùng một đoạn code — script dày lên là chỗ trốn của những
nhánh không test nào chạm tới.
...
def main() -> int:
    ...
    result = train_bagging(params, seed=args.seed, n_splits=args.n_splits,
    ...
    print(f"\n=== Bagging · CV {result['config']['n_splits']}-fold trên "
